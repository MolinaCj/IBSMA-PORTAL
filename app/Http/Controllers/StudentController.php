...
// Display a listing of the resource.
     ...
     $students = Student::where('graduated', '!=', 1) // Exclude graduated students
         ...
         foreach ($students as $student) {
            foreach ($student->subjects as $subject) {
                ...
                $subject->gpa = $grade !== null ? $this->percentageToGPA($grade) : 'N/A';
            }
        }
 ...
 public function records(Request $request)
 {
     ...
     $students = Student::select('students.*', 'graduates.graduation_year')->leftJoin('graduates', 'graduates.student_id', '=', 'students.student_id')
     ->when($search, function ($query, $search) {
             $query->where(function ($q) use ($search) {
                 $q->where('last_name', 'like', '%' . $search . '%')
                   ...
                   ->orWhere('graduates.graduation_year', '=', $search );
             });
         })
        ->when($filter == 'graduates', function ($q, $filter) {
            return $q->where('graduated', '=', 1);
        })
        ->when($filter == 'undergraduates', function ($q, $filter) {
     ...
     return view('admin.records', compact('students', 'departments','teachers','currentSemester', 'lastStudentId'));
 }
...
public function store(Request $request)
{
    try {
        ...
        $complete_address = "";
        if(!empty($request->barangay_name)){
            $complete_address = $request->barangay_name;
        }
        if(!empty($request->city_name)){
            $complete_address = $complete_address.", ".$request->city_name;
        }
        if(!empty($request->province_name)){
            $complete_address = $complete_address.", ".$request->province_name;
        }
        if(!empty($complete_address)){
            $validatedData['complete_address'] = $complete_address;
        }
    ...
    public function updateGrades(Request $request, $studentId)
    {
        ...
        foreach ($request->subjects as $subjectId => $data) {
            // Find the latest record for the given student_id and subject_id
            $latestGrade = Grade::where('student_id', $student->student_id)
                ->where('subject_id', $subjectId)
                ->orderBy('created_at', 'desc') // Ensure the latest record is selected
                ->first();
        
            // Update only the latest record
            if ($latestGrade) {
                $latestGrade->update([
                    'grade' => $data['grade'] ?? null, // Use null if 'grade' key is missing
                    'credited' => $data['credited'] ?? 0, // Update credited status
                ]);
            }
        }
    ...
    private function percentageToGPA($grade)
    {
    ...
    public function recheckStudentSubjects($studentId)
{
    ...
    public function updateTor(Request $request, $studentId)
    {     
        ...
        $validated = $request->validate([
            ...
            'student_picture_file' => 'image|mimes:jpeg,png,jpg,gif|max:20480',
        ...
        $tor = Tor::where('student_id', '=', $studentId);

        if(!empty($request->student_picture_file)){
            $imageName = time().'_'.$student->student_id.'.'.$request->student_picture_file->extension();
            $request->student_picture_file->move(public_path('images'), $imageName);
            if(!empty($request->student_picture) && File::exists(public_path($request->student_picture))) unlink(public_path($request->student_picture));

        } else {
            $imageName = str_replace("images/", "", $request->student_picture);
        }
        try {
            if (!$tor->exists()) {
                data_forget($validated, 'student_picture_file');
                $newTor = new Tor($validated);
                $newTor->student_picture = 'images/'.$imageName;
                $newTor->student()->associate($student)->save();
            } else {
                $validated['student_picture'] = 'images/'.$imageName;
                data_forget($validated, 'student_picture_file');
                $tor->update($validated);
            }
        } catch (Exception $e) {
        ...
        $student->transferee = $request->transferee ? 1 : 0;
        $student->save();

        FormerSubject::where('student_id', '=', $studentId)->delete();

        //remove all former subjects to be replaced by a new set of subjects
        if($request->transferee) {        
            $request->validate([
                'additionalSubjects.*.school_name' => 'required|string|max:255',
                'additionalSubjects.*.school_year' => 'required',
                'additionalSubjects.*.year' => 'required|integer|between:1,4',
                'additionalSubjects.*.semester' => 'required|integer|between:1,3',
                'additionalSubjects.*.code' => 'required|string|max:255',
                'additionalSubjects.*.title' => 'required|string',
                'additionalSubjects.*.grade' => 'required',
                'additionalSubjects.*.credits' => 'required',
                'additionalSubjects.*.remarks' => 'required|string|max:255',
            ]);


            foreach ($request->additionalSubjects as $key => $value) {
                $prevSubject = new FormerSubject($value);
                $prevSubject->student()->associate($student)->save();
            }
        }
        return response()->json([
            'success' => true,
    ...
    public function downloadTor($studentId){
        $student = Student::find($studentId);
    
        $tor = Tor::where('student_id', $studentId)->get();

        $formerSubjects = FormerSubject::where('student_id', '=', $studentId)
        ->orderBy('created_at', 'desc')
        ->get();
    
        $torSubjects = $student->subjects()->where('credited', '!=', 1)->get();

        $pdf = Pdf::loadView('admin.tor', compact('student', 'tor', 'formerSubjects', 'torSubjects'))
        ->setPaper('Legal', 'portrait')
        ->setOption([
            'fontDir' => public_path('/fonts'),
            'fontCache' => public_path('/fonts'),
            'defaultFont' => 'Old English'
        ]);
        $studentName = $student->last_name.", ".$student->first_name.' '.$student->middle_name;
        $timestamp = strtotime("now");
        $filename = "IBSMA Transcript of Records - $studentName $timestamp.pdf";
...
public function update(Request $request, Student $student)
{
    ...
    if(!empty($complete_address)){
        $student->complete_address = $complete_address;
        $student->save();
    }
...
public function updateRecords(Request $request, Student $student)
{
    // Validate the incoming data
    $validated = $request->validate([
        'student_id' => 'required|string|max:255',
