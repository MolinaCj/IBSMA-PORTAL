    ...
    if ($request->has('grades')) {
        ...
        foreach ($gradesData as $studentId => $subjects) {
            foreach ($subjects as $subjectId => $gradeValue) {
                ...
                // Find the latest grade for this student and subject
                $latestGrade = Grade::where('student_id', $studentId)
                    ->where('subject_id', $subjectId)
                    ->orderBy('school_year', 'desc') // Ensure the latest school year is selected
                    ->orderBy('id', 'desc') // Fallback to ID if school_year is the same
                    ->first();
                ...
                if (!$latestGrade) {
                    ...
                    // Update only the latest grade
                    $latestGrade->update([
                        'grade' => $gradeValue,
                        'incomplete' => ($gradeValue >= 75) ? 0 : $latestGrade->incomplete, // Update incomplete status
                    ]);
                }
            }
        }
        ...
        // Find the latest grade for this student and subject
        $latestGrade = Grade::where('student_id', $request->student_id)
        ...
        if (!$latestGrade) {
            ...
            $latestGrade->update([
                'grade' => $request->grade,
                'incomplete' => ($request->grade >= 75) ? 0 : $latestGrade->incomplete, // Update incomplete status
            ]);
        }
