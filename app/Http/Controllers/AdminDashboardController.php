...
public function updateSemester(Request $request)
{
    ...
    foreach ($students as $student) {
        ...
        // Get all grades and keep only the latest per subject
        $latestGrades = Grade::where('student_id', $student->student_id)
            ->orderBy('school_year', 'desc')
            ->orderBy('id', 'desc')
            ->get()
            ->unique('subject_id');

        // Identify failed or null-graded subjects for this semester
        $failedSubjects = $latestGrades->filter(function ($grade) use ($request) {
            return (($grade->grade === null || $grade->grade < 74.5) && $grade->credited != 1) && $grade->semester == $request->semester;
        ...
        // Determine passed subjects from the latest grades
        // Add Next Eligible Subjects (After Passing Prerequisites)
        $passedSubjects = $latestGrades->filter(function ($grade) {
            return ($grade->grade !== null && $grade->grade >= 74.5) || $grade->credited;
        })->pluck('subject_id')
        ->toArray();
        ...
        if ($request->semester == 3) {
            $summerSubjects = Subject::where('semester', 3)
                ->where('department_id', $student->department_id)
                ->where('year', '<=', $student->year_level) // Added year level filter
                ->get();
    ...
    return redirect()->back()->with('success', 'Semester updated successfully. Subjects re-evaluated based on most recent grades.');
}
