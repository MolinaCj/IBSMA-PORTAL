                            ...
                            <div class="row g-2 mt-2">
                                ...
                                <div class="col-md-6">
                                    ...
                                    @error('region')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                    <input name="region_name" id="regionName" type="hidden" />
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    @error('province')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                    <input name="province_name" id="provinceName" type="hidden" />
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    @error('city')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                    <input name="city_name" id="cityName" type="hidden" />
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    @error('barangay')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                    <input name="barangay_name" id="barangayName" type="hidden" />
                                </div>
                            </div>
                        </div>
...
@endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const barangayInput = document.getElementById('barangayName');
                                    ...
                                    <table class="custom-table">
                                        ...
                                        <tbody>
                                            ...
                                            @php
                                                ...
                                                $grade = $subject->pivot->grade ?? null;
                                                $gradeRecord = \App\Models\Grade::where('student_id', $student->student_id)
                                                    ->where('subject_id', $subject->id)
                                                    ->orderBy('created_at', 'desc')
                                                    ->first();
                                                    
                                                $credited = $gradeRecord ? (int)$gradeRecord->credited : 0;
                                            @endphp
                                            <tr class="{{ $highlightRed ? 'table-danger' : '' }}">
                                                ...
                                                <td id="gpa-cell-{{ $subject->id }}" style="width: 60px; text-align: center;">
                                                    {{ $subject->gpa ?? 'N/A' }}
                                                </td>
                                                <td style="white-space: nowrap;">
                                                    <div style="display: flex; align-items: center; gap: 8px;">
                                                        ...
                                                        <input type="number" name="subjects[{{ $subject->id }}][grade]"
                                                            class="grade-input {{ isset($grade) && $grade < 74.50 ? 'border-danger' : '' }}"
                                                            value="{{ $grade }}" placeholder="Enter grade" step="0.01"
                                                            {{ $credited === 1 ? '' : 'disabled' }}>
                                                            
                                                        {{-- Hidden Input for Credited --}}
                                                        <input type="hidden" name="subjects[{{ $subject->id }}][credited]" value="{{ $credited }}">
                                                            
                                                        {{-- Toggle Switch with strict condition --}}
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input credit-toggle" type="checkbox"
                                                                data-subject-id="{{ $subject->id }}" {{ $credited === 1 ? 'checked' : '' }}>
                                                        ...
                                                        @if(isset($grade))
                                                            ...
                                                            @else
                                                                <span class="badge bg-success text-white fw-bold">Passed</span>
                                                            @endif
                                                        @endif
                                                        
                                                        {{-- Credited Label with strict condition --}}
                                                        @if($credited === 1)
                                                            <span style="background: orange; color: black; padding: 2px 6px;">Credited</span>
                                                        @endif
                                                    </div>
                                                    
                                                    {{-- <script>
                                                        document.addEventListener('DOMContentLoaded', function () {
                                                            // Fix initial state of all toggles and labels
                                                            document.querySelectorAll('input[name^="subjects"][name$="[credited]"]').forEach(creditedInput => {
                                                                const subjectId = creditedInput.name.match(/subjects\[(\d+)\]/)[1];
                                                                const toggle = document.querySelector(`.credit-toggle[data-subject-id="${subjectId}"]`);
                                                                const gradeInput = document.querySelector(`input[name="subjects[${subjectId}][grade]"]`);
                                                                const creditedLabel = toggle.closest('div.form-check.form-switch').parentElement.querySelector('span[style*="background: orange"]');
                                                                
                                                                // Set toggle state based on hidden input value
                                                                toggle.checked = creditedInput.value === "1";
                                                                
                                                                // Enable/disable grade input based on toggle state
                                                                if (toggle.checked) {
                                                                ...
                                                                // Show/hide credited label based on toggle state
                                                                if (creditedLabel) {
                                                                    creditedLabel.style.display = toggle.checked ? '' : 'none';
                                                                }
                                                            });
                                                    
                                                            // Add change event listeners to all toggles
                                                            document.querySelectorAll('.credit-toggle').forEach(toggle => {
                                                                toggle.addEventListener('change', function () {
                                                                    const subjectId = this.dataset.subjectId;
                                                                    const gradeInput = document.querySelector(`input[name="subjects[${subjectId}][grade]"]`);
                                                                    const creditedInput = document.querySelector(`input[name="subjects[${subjectId}][credited]"]`);
                                                                    const creditedLabel = this.closest('div.form-check.form-switch').parentElement.querySelector('span[style*="background: orange"]');
                                                                    
                                                                    if (this.checked) {
                                                                        gradeInput.removeAttribute('disabled');
                                                                        creditedInput.value = 1;
                                                                        if (creditedLabel) {
                                                                            creditedLabel.style.display = '';
                                                                        }
                                                                    } else {
                                                                        gradeInput.setAttribute('disabled', 'disabled');
                                                                        creditedInput.value = 0;
                                                                        if (creditedLabel) {
